...
GALLERY_DIR = os.path.join(BASE_DIR, "test-2", "gallery")
QUERY_DIR = os.path.join(BASE_DIR, "test-2", "query")
...
class GeM(nn.Module):
    ...
    """
    def _init_(self, p=3.0, eps=1e-6):
        super()._init_()
...
class CLIPGeMArcFace(nn.Module):
    ...
    """
    def _init_(self, clip_model, embed_dim=1024, num_classes=None, unfreeze_layers=4):
        super()._init_()
...
class CenterLoss(nn.Module):
    ...
    """
    def _init_(self, num_classes, feat_dim):
        super()._init_()
...
class ArcFaceLoss(nn.Module):
    def _init_(self, scale=64.0, margin=0.5):
...
class ImprovedIdentityDataset(Dataset):
    def _init_(self, root_dir, preprocess_fn, is_training=True):
        self.samples = []
        self.labels = []
        self.label_map = {}
...
class FlatTestDataset(Dataset):
    def _init_(self, root_dir, preprocess_fn):
